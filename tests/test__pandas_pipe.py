...
from pipepy.pandas_pipe import category_to_numeric, CategoryToNumericPipe, DropColumnPipe, MapColumnPipe
...
class CategoryToNumericPipeTest(unittest.TestCase):
    ...
    def test_category_to_numeric_pipe(self):
        ...
        assert_array_equal(pipe.flush(df), np.asarray([[0, 1, 0, 3], [1, 5, 1, 7]]))
...
class DropColumnPipeTest(unittest.TestCase):
    ...
    def test_drop_columns(self):
        ...
        assert_array_equal(pipe.flush(df), np.asarray([[1], [5]]))

    def test_drop_columns_colnames(self):
        df = pd.DataFrame(np.asarray([[0, 1, 2, 3], [4, 5, 6, 7]]), columns=['one', 'two', 'three', 'four'])
        pipe = DropColumnPipe(['one', 'three'])

        assert_array_equal(pipe.flush(df), np.asarray([[1, 3], [5, 7]]))
...
class ApplyOnAxisPipeTest(unittest.TestCase):
    ...
    def test_apply_on_axis(self):
        ...
        assert_array_equal(pipe.flush(df), np.asarray([[1, 1, 2, 4], [5, 5, 6, 8]]))
